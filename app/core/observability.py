...
def setup_logfire(app) -> None:
    ...
    logfire.configure(
        ...
        console=logfire.ConsoleOptions(min_log_level="warn") if os.getenv("LOGFIRE_CONSOLE", "false").lower() != "true" else True,
    )

    logfire.instrument_fastapi(
        app,
        excluded_urls=os.getenv("LOGFIRE_EXCLUDED_URLS", "/health"),
        capture_headers=os.getenv("LOGFIRE_CAPTURE_HEADERS", "false").lower() == "true",
    )
    logfire.instrument_httpx(capture_headers=os.getenv("LOGFIRE_CAPTURE_HEADERS", "false").lower() == "true")
